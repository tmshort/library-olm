...
| C10 | Not a live OLMv0-managed dependency *(hard)* | Subscription carries the `olm.generated-by` annotation **and** the parent operator's Subscription still exists on the cluster — migrating while the parent is still on OLMv0 would cause OLMv0 to see an unresolved dependency and attempt to reinstall the operator | none — migrate the parent operator first (or verify it is already migrated); if the parent's Subscription is gone the dependency can be migrated freely |
...
| annotation `olm.generated-by` | — | If present, the Subscription was auto-generated by OLMv0 to satisfy another operator's dependency → apply C10: block migration if the parent's Subscription still exists, allow if the parent is already migrated (no parent Subscription on cluster). |
